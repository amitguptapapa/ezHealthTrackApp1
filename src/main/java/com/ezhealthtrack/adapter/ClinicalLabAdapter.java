...
public class ClinicalLabAdapter extends ArrayAdapter<Appointment> implements
		Filterable {
	...
	Filter filter = new Filter() {
		...
		protected void publishResults(final CharSequence constraint,
			...
			try {
				ClinicalLabAdapter.objects.clear();
				ClinicalLabAdapter.objects
	...
	public ClinicalLabAdapter(final Context context, final int resourceId,
			final ArrayList<Appointment> item) {
		super(context, resourceId, ClinicalLabAdapter.objects);
		sharedPref = context.getSharedPreferences(Constants.EZ_SHARED_PREF,
				Context.MODE_PRIVATE);
		this.context = context;
		confirmedList = item;
		ClinicalLabAdapter.objects.clear();
		ClinicalLabAdapter.objects.addAll(item);
	...
	public View getView(final int position, final View convertView,
		...
		View row = null;
		try {
			final Appointment rowItem = ClinicalLabAdapter.objects
					.get(position);
			if (convertView == null) {
