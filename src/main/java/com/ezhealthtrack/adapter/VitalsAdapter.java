...
public class VitalsAdapter extends ArrayAdapter<Appointment> implements
		Filterable {
	...
	Filter filter = new Filter() {
		...
		protected void publishResults(final CharSequence constraint,
			...
			try {
				VitalsAdapter.objects.clear();
				VitalsAdapter.objects
	...
	public VitalsAdapter(final Context context, final int resourceId,
			final ArrayList<Appointment> item) {
		super(context, resourceId, VitalsAdapter.objects);
		sharedPref = context.getSharedPreferences(Constants.EZ_SHARED_PREF,
				Context.MODE_PRIVATE);
		this.context = context;
		confirmedList = item;
		VitalsAdapter.objects.clear();
		VitalsAdapter.objects.addAll(item);
	...
	public View getView(final int position, final View convertView,
		...
		View row = null;
		try {
			final Appointment rowItem = VitalsAdapter.objects.get(position);
			if (convertView == null) {
